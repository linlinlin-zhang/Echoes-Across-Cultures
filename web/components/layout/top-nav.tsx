  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 px-4 py-3 md:px-8">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 rounded-2xl panel-glass px-4 py-2.5">
        <button className="group flex items-center gap-2" onClick={() => onNavigate(items[0]?.id ?? 'overview')} aria-label={isZh ? '返回总览' : 'Go to overview'}>
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label={isZh ? '主导航' : 'Primary'}>
          {items.map((item) => (
            <button
              key={item.id}
