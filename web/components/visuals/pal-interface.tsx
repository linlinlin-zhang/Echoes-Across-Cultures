  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
        <h3 className="font-display text-2xl text-textMain">Uncertainty Heatmap</h3>
        <p className="mb-4 text-sm text-textSub">Darker cells indicate samples where the model asks for expert intervention.</p>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  selectedCell ? 'border-zs shadow-neon' : 'border-white/10'
                )}
                style={{
                  background: `rgba(165,94,234,${0.12 + intensity * 0.82})`
                }}
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/25 p-4">
          <div className="font-display text-lg text-textMain">Feedback Loop Dynamics</div>
          <p className="text-xs text-textSub">Edges tighten as expert labels increase confidence in local ontology neighborhoods.</p>

          <svg viewBox="0 0 400 140" className="mt-3 h-36 w-full rounded-xl border border-white/10 bg-black/40">
      ...
      <div className="space-y-4">
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          <h3 className="font-display text-2xl text-textMain">Expert Annotation Panel</h3>
          ...
          <div className="mt-4 space-y-3">
            <label className="block">
              ...
              <select
                value={affectLabel}
                onChange={(event) => setAffectLabel(event.target.value)}
            ...
            <label className="block">
              ...
              <input
                value={cultureLabel}
                onChange={(event) => setCultureLabel(event.target.value)}
                className="w-full rounded-xl border border-white/20 bg-black/35 px-3 py-2 text-sm text-textMain"
              />
            </label>
            ...
            <label className="block">
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-white/20 bg-black/35 px-3 py-2 text-sm text-textMain"
                placeholder="Explain cultural context, performance practice, or affect semantics..."
              />
            </label>

            <button
              onClick={submitAnnotation}
              className="inline-flex items-center rounded-full bg-zs px-4 py-2 text-sm font-semibold text-abyss"
            >
          ...
          <div className="mt-4 space-y-2 text-xs text-textSub">
            ...
            {annotations.map((item, index) => (
              <div key={`${item.sampleId}-${index}`} className="rounded-lg border border-white/10 bg-black/20 px-2 py-1.5">
        ...
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          <h4 className="font-display text-lg text-textMain">Ontology Expansion</h4>
          <div className="mt-2 flex gap-2">
            <input
              value={newConcept}
              onChange={(event) => setNewConcept(event.target.value)}
              placeholder="Add concept node (e.g. Han, Saudade)"
              className="flex-1 rounded-xl border border-white/20 bg-black/35 px-3 py-2 text-sm text-textMain"
            />
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {concepts.map((concept) => (
              <span key={concept} className="rounded-full border border-white/20 bg-black/25 px-2 py-1 text-xs text-textSub">
                {concept}
              </span>
            ))}
          </div>
          ...
          <div className="mt-4">
            <div className="mb-2 text-xs text-textSub">Cognitive Justice Indicator (ontology coverage)</div>
