    ...
    <div className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          <h3 className="font-display text-2xl text-textMain">A/B Recommendation Comparison</h3>
          <p className="mb-4 text-sm text-textSub">Baseline vs DDRL-OT recommendation lists for the same user preference profile.</p>

          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-black/25 p-3">
              <div className="mb-2 font-display text-sm text-textMain">Conventional Recommender</div>
              <ul className="space-y-1 text-xs text-textSub">
                {baselineRecommendations.map((item) => (
                  <li key={`b-${item.rank}`} className="flex items-center justify-between">
                    <span>
            ...
            <div className="rounded-xl border border-zs/40 bg-zs/10 p-3">
              <div className="mb-2 font-display text-sm text-textMain">Soundscape Without Borders</div>
              <ul className="space-y-1 text-xs text-textSub">
                {dcasRecommendations.map((item) => (
                  <li key={`d-${item.rank}`} className="flex items-center justify-between">
                    <span>
                      {item.rank}. {item.title}
                    </span>
                    <span>{item.culture}</span>
        ...
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          <h3 className="font-display text-2xl text-textMain">Serendipity Calculator</h3>
          <p className="mb-3 text-sm text-textSub">Serendipity = Unexpectedness (zs distance) × Relevance (za proximity)</p>
          ...
          <div className="mt-4 rounded-xl border border-white/10 bg-black/25 p-4">
            <p className="font-mono text-sm text-textSub">{unexpectedness.toFixed(2)} × {relevance.toFixed(2)} =</p>
            <p className="font-display text-4xl text-zs">{serendipity.toFixed(3)}</p>
          </div>

          <div className="mt-4">
            <div className="mb-1 flex items-center justify-between text-xs text-textSub">
              <span>Decolonization Index</span>
              <span>{(decolonizationIndex * 100).toFixed(1)}%</span>
            </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
          ...
          <div className="mt-3 h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={fairnessData}>
                ...
                <XAxis dataKey="culture" tick={{ fill: '#cbd5e1', fontSize: 11 }} />
        ...
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          ...
          <div className="mt-3 h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={95} fill="#4ecdc4" label />
                <Tooltip
                  contentStyle={{
                    background: '#0f172a',
                    border: '1px solid rgba(148,163,184,.4)'
