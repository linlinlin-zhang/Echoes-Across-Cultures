...
export function DisentanglementLab() {
  ...
  useSpectrumCanvas(processedCanvasRef, processedSpectrum, '#00a7a0')
  useSpectrumCanvas(originalCanvasRef, originalSpectrum, '#ff6f61')
  ...
  const initializeAudio = async () => {
    ...
    setAudioReady(true)
    setStatusText('Audio context ready. Press Play to hear factor-controlled transfer.')
  }
  ...
  const stopPattern = () => {
    ...
    setStatusText('Playback stopped. Adjust sliders and replay.')
  }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.96fr_1.04fr]">
      <div className="space-y-4 rounded-3xl paper-card p-5">
        <div>
          <span className="chapter-chip">chapter worksheet</span>
          <h3 className="mt-2 font-display text-3xl text-textMain">Style Transfer Workbench</h3>
          <p className="mt-2 text-sm text-textSub">Step through source selection, latent tuning, and realtime interpretation views.</p>
        </div>

        <div className="grid gap-3">
          <div className="note-card">
            <p className="font-mono text-[11px] uppercase tracking-[0.13em] text-textSub">step 1 · source</p>
            ...
            {uploadedName ? <p className="mt-2 font-mono text-xs text-textSub">Loaded: {uploadedName}</p> : null}
          </div>

          <div className="note-card">
            <p className="font-mono text-[11px] uppercase tracking-[0.13em] text-textSub">step 2 · latent tuning</p>
          ...
          <div className="note-card">
            <p className="font-mono text-[11px] uppercase tracking-[0.13em] text-textSub">step 3 · audition</p>
            <div className="mt-2 flex flex-wrap gap-2">
              <button onClick={initializeAudio} className="rounded-full border border-zs/50 bg-zs/10 px-4 py-2 text-sm font-semibold text-zs">Initialize</button>
              <button onClick={playPattern} disabled={!audioReady && isPlaying} className="rounded-full bg-zc px-4 py-2 text-sm font-semibold text-white">Play</button>
              <button onClick={stopPattern} className="rounded-full border border-ink/25 bg-white px-4 py-2 text-sm font-semibold text-textMain">Stop</button>
            </div>
      ...
      <div className="space-y-4">
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="paper-card rounded-2xl p-3">
            <h4 className="font-display text-base text-textMain">Original Spectrum</h4>
            <canvas ref={originalCanvasRef} width={420} height={180} className="mt-2 h-44 w-full rounded-lg border border-ink/15" />
          </div>
          <div className="paper-card rounded-2xl p-3">
            <h4 className="font-display text-base text-textMain">Transferred Spectrum</h4>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="paper-card rounded-2xl p-4">
            ...
            <svg viewBox="0 0 100 80" className="h-28 w-full rounded-lg bg-[#fff8ee]">
              ...
              {pianoPattern.map((note, idx) => (
                <rect key={`n-${idx}`} x={note.x} y={note.y} width={note.w} height={5.5} fill="#ff6f61" opacity={0.86} rx={1.2} />
              ))}
            </svg>
          </div>
          ...
          <div className="paper-card rounded-2xl p-4">
            ...
            <div className="relative h-28 rounded-lg border border-ink/15 bg-[#fff8ee]">
              ...
              <div className="absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-za shadow-[0_0_12px_rgba(126,87,194,0.6)]" style={{ left: `${(valence + 1) * 50}%`, top: `${(1 - (arousal + 1) / 2) * 100}%` }} />
            </div>
            <p className="mt-2 font-mono text-xs text-textSub">V={valence.toFixed(2)} · A={arousal.toFixed(2)}</p>
          </div>

          <div className="paper-card rounded-2xl p-4">
            <h4 className="font-display text-base text-textMain">Culture Similarity</h4>
            ...
            <div className="relative mx-auto h-28 w-28">
              <svg viewBox="0 0 120 120" className="h-full w-full -rotate-90">
                ...
                <circle cx="60" cy="60" r="44" fill="none" stroke="#00a7a0" strokeWidth="10" strokeLinecap="round" strokeDasharray={`${Math.PI * 2 * 44 * cosineSimilarityToWestern} ${Math.PI * 2 * 44}`} />
              </svg>
