...
说明：本文件是 `part_discussion_refs.md` 的讲解补充版。重点补充讨论部分里较抽象的方法论概念、伦理术语，以及参考文献在整篇论文中分别提供了什么理论支撑。
...
**补充讲解（这里的“泛化能力”不只是换测试集）**：在传统机器学习里，泛化常常被理解成“换一批测试样本还能保持精度”。但对多目标推荐系统而言，泛化还包括另一个层面：当骨干模型变了、数据偏差强度变了、部署目标变了时，系统是否仍然保持可解释、可调节的行为模式。DCAS 在讨论部分强调的正是这种更广义的泛化。
...
**补充讲解（什么叫“有限贡献”）**：这并不是说 GRL 没有理论价值，而是说在当前框架里，它的边际收益没有其他模块那么强。研究里非常重要的一点，就是要区分“理论上合理”和“在当前系统中最关键”这两件事。很多模块可能都合理，但真正决定性能轮廓的，往往只有其中几项。把这一点说清楚，能帮助后续研究把资源投入到更值得优化的地方。
...
**补充讲解（为什么数据建设常常比模型技巧更根本）**：如果数据层面天然把“文化”和“来源”绑死了，那么模型再复杂，也只是在一个有缺陷的世界里做更精细的补偿。方法创新当然重要，但它有时更像是在为数据基础设施还债。因此本文把 Source Confound 当作核心限制来讨论，是非常有分量的方法论自觉。
...
**补充讲解（为什么“以人为中心”不是一句口号）**：所谓以人为中心，并不只是最后找用户做个问卷，而是承认模型并不天然懂文化，很多关键判断需要持续与人类专家、社区实践者共同协商。对跨文化音乐来说，这一点尤其重要，因为“相似”“合适”“尊重”这些判断往往没有唯一标准答案，而是深深嵌在具体文化语境中。
...
**补充讲解（这对 MIR 社区意味着什么）**：这段启示背后其实有一个更大的研究方向变化。过去 MIR 很多工作强调“识别更准”“检索更快”，而本文所代表的一类工作开始强调“表征是否带偏差”“推荐是否可治理”“系统是否能解释自己的文化取舍”。这意味着 MIR 正在从纯技术优化问题，逐渐走向兼顾技术、社会与文化责任的复合研究领域。
...
**补充讲解（伦理部分为什么和方法强相关）**：很多人会把伦理声明看成论文最后的附属章节，但在跨文化推荐中，它其实直接关系到方法本身是否成立。因为你怎么定义文化类别、谁来标注、哪些文化被纳入、如何理解公平，都会反过来塑造模型的训练目标与输出行为。也就是说，这里的伦理不是“做完技术后顺手交代”，而是技术设计的内在组成部分。
...
这也说明为什么未来工作会强调连续文化表征。离散标签在实验里方便，但现实中的文化往往是连续、混合、演化的。如果研究长期停留在硬边界分类上，就可能无意间把原本流动的文化现象讲得过于僵硬。
...
**补充讲解（为什么参与式方法也会带来新风险）**：很多人会天然觉得“有专家参与”就一定更公平，但事情没这么简单。谁被邀请参与、谁有时间参与、谁的话更容易被系统采纳，本身就可能形成新的不平等结构。所以参与式方法不是自动的伦理保证，它也需要制度设计和透明治理。
...
如果把这三条缓解措施放回整篇论文的脉络里看，就会发现它们与 DCAS 的方法设计是一致的：显式权重、可解释操作点、可引入专家反馈、可报告源混淆。这意味着论文的目标不仅是“做出更强模型”，也是“做出更容易被审查和协商的系统”。
...
**补充讲解（未来工作如何归类理解）**：这些未来方向大致可以分成三类。第一类是表示层深化，例如动态文化表征和多模态融合；第二类是系统层落地，例如在线学习和跨语言推荐；第三类是研究基础设施建设，例如用户研究和源混淆报告标准。这样理解后，会更容易把“未来工作”写成一张清晰的研究路线图，而不只是若干松散展望。
...
下面这部分不是新增引用，而是帮助理解这些文献在本文中分别承担什么作用。

1. **基础表征类**：`MERT`、`CultureMERT`、`GlobalMood`、`Universal Music Representations?`
这些文献定义了本文所站立的“听音频”基础，也解释了为什么跨文化表征本身就是一个独立难题。

2. **解耦学习类**：`VAE`、`beta-VAE`、`FactorVAE`、`Isolating Sources of Disentanglement`
这些文献提供了潜变量建模、独立性约束和解耦理论框架，是 DCAS 在表示层最核心的学术来源。

3. **推荐与排序类**：`BPR`、`LambdaMART`、`Calibrated Recommendations`、`Auralist`
这些工作决定了本文如何理解推荐目标，不只是要相关，还要校准、排序合理、具有惊喜度。

4. **最优传输类**：`Sinkhorn Distances`、`Computational Optimal Transport`、OT 在推荐中的迁移或公平应用
这些文献支撑了本文将分布对齐引入跨文化推荐的做法，说明 OT 并非临时拼接，而是有完整数学传统的工具。

5. **伦理与参与式方法类**：`Ethical Dimensions of MIR`、`Responsible MIR`、`Human-in-the-Loop`
这些工作帮助本文把模型性能之外的文化责任、参与式标注和社区协商纳入研究议程。
