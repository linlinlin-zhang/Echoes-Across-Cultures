...
def load_ablation_comparison(suite: str, name: str) -> Optional[dict]:
    return load_json(
        BASE_DIR / "reports" / "audits" / "ablation_v4_main_2026-04-05" / suite / f"comparison_{name}.json"
    )
...
# ==============================================================================
def _mc(m):
    if "calibrated_minor" in m:
        return DCAS_CAL_MINOR
    if "calibrated_target" in m:
        return DCAS_CAL_TARGET
    if "dcas" in m:
        return DCAS_OT
    if "lambdamart" in m:
        return BPR_LM
...
def fig01_v4_cm_main():
    ...
    for ax, met, lab in zip(axes, mets, labs):
        ...
        ax.set_yticklabels([mzh(m) for m in df["method"]])
        ax.set_xlabel(lab)
        ax.set_title(lab)
        rng = max(vals) - min(vals) + 0.01
        for bar, v in zip(bars, vals):
            ax.text(
                bar.get_width() + 0.005 * rng,
    ...
    save_fig(fig, "v4_cm_main_results_3panel")
    save_csv(df[["method"] + mets], "v4_cm_main_results_3panel")
...
def fig02_v4_gm_main():
    ...
    save_fig(fig, "v4_gm_main_results_3panel")
    save_csv(df[["method"] + mets], "v4_gm_main_results_3panel")
...
def fig03_routeA_cm():
    ...
    save_fig(fig, "routeA_cm_main_results")
    save_csv(df[["method"] + mets], "routeA_cm_main_results")
...
def fig04_routeA_gm():
    ...
    save_fig(fig, "routeA_gm_main_results")
    save_csv(df[["method"] + mets], "routeA_gm_main_results")
...
def fig05_public_cn():
    ...
    save_fig(fig, "public_cn_results")
    save_csv(df[["method"] + mets], "public_cn_results")
...
def fig06_serendipity_cm_all():
    ...
    ax.set_yticklabels([mzh(m) for m in df["method"]])
    ax.set_xlabel("惊喜度")
    ax.set_title("V4 CultureMERT 各方法惊喜度对比")
    rng = max(vals) - min(vals) + 0.01
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.005 * rng,
    ...
    ax.invert_yaxis()
    save_fig(fig, "serendipity_all_methods_cm")
    save_csv(df[["method", "serendipity_mean"]], "serendipity_all_methods_cm")
...
def fig07_serendipity_gm_all():
    ...
    ax.set_yticklabels([mzh(m) for m in df["method"]])
    ax.set_xlabel("惊喜度")
    ax.set_title("V4 Gemini 各方法惊喜度对比")
    rng = max(vals) - min(vals) + 0.01
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.005 * rng,
    ...
    ax.invert_yaxis()
    save_fig(fig, "serendipity_all_methods_gm")
    save_csv(df[["method", "serendipity_mean"]], "serendipity_all_methods_gm")
...
def fig08_minority_cm_all():
    ...
    ax.set_yticklabels([mzh(m) for m in df["method"]])
    ax.set_xlabel("少数曝光率")
    ax.set_title("V4 CultureMERT 各方法少数曝光率")
    rng = max(vals) - min(vals) + 0.01
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.005 * rng,
    ...
    ax.invert_yaxis()
    save_fig(fig, "minority_exposure_all_methods_cm")
    save_csv(
        df[["method", "minority_exposure_at_k_mean"]],
        "minority_exposure_all_methods_cm",
    )
...
def fig09_minority_gm_all():
    ...
    ax.set_yticklabels([mzh(m) for m in df["method"]])
    ax.set_xlabel("少数曝光率")
    ax.set_title("V4 Gemini 各方法少数曝光率")
    rng = max(vals) - min(vals) + 0.01
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.005 * rng,
    ...
    ax.invert_yaxis()
    save_fig(fig, "minority_exposure_all_methods_gm")
    save_csv(
        df[["method", "minority_exposure_at_k_mean"]],
        "minority_exposure_all_methods_gm",
    )
...
def fig10_calibkl_cm_all():
    ...
    ax.set_yticklabels([mzh(m) for m in df["method"]])
    ax.set_xlabel("校准KL")
    ax.set_title("V4 CultureMERT 各方法校准KL")
    rng = max(vals) - min(vals) + 0.01
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.005 * rng,
    ...
    ax.invert_yaxis()
    save_fig(fig, "calibration_kl_all_methods_cm")
    save_csv(df[["method", "cultural_calibration_kl_mean"]], "calibration_kl_all_methods_cm")
...
def fig11_calibkl_gm_all():
    ...
    ax.set_yticklabels([mzh(m) for m in df["method"]])
    ax.set_xlabel("校准KL")
    ax.set_title("V4 Gemini 各方法校准KL")
    rng = max(vals) - min(vals) + 0.01
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.005 * rng,
    ...
    ax.invert_yaxis()
    save_fig(fig, "calibration_kl_all_methods_gm")
    save_csv(df[["method", "cultural_calibration_kl_mean"]], "calibration_kl_all_methods_gm")
...
def fig12_cross_serp():
                ...
                rows.append(
                    {
                        "基准": slbl,
                        "方法": mzh(m),
                        "惊喜度": df.loc[df["method"] == m, "serendipity_mean"].values[0],
                    }
                )
    if not rows:
        print("  SKIPPED")
    ...
    cmap = {
        "DCAS-少数导向": DCAS_CAL_MINOR,
        "DCAS-目标校准": DCAS_CAL_TARGET,
        "DCAS-OT": DCAS_OT,
        "BPR-LM": BPR_LM,
        "热门度": POP_ORANGE,
    }
    ...
    for i, col in enumerate(avail):
        ax.bar(
            ...
            w,
            label=col,
            color=cmap.get(col, "#888"),
            zorder=3,
        )
    ax.set_xticks(x)
    ax.set_xticklabels(piv.index)
    ax.set_ylabel("惊喜度")
    ax.set_title("跨4基准惊喜度对比")
    ax.legend(fontsize=8)
    ax.grid(axis="y", alpha=0.3)
    save_fig(fig, "cross_suite_serendipity")
    save_csv(rdf, "cross_suite_serendipity")
...
def fig13_cross_minor():
                ...
                rows.append(
                    {
                        "基准": slbl,
                        "方法": mzh(m),
                        "少数曝光率": df.loc[df["method"] == m, "minority_exposure_at_k_mean"].values[0],
                    }
                )
    if not rows:
        print("  SKIPPED")
    ...
    ax.set_xticklabels(piv.index)
    ax.set_ylabel("少数曝光率")
    ax.set_title("跨4基准少数曝光率对比")
    ax.legend(fontsize=8)
    ax.grid(axis="y", alpha=0.3)
    save_fig(fig, "cross_suite_minority")
    save_csv(rdf, "cross_suite_minority")
...
def fig14_cross_calib():
                ...
                rows.append(
                    {
                        "基准": slbl,
                        "方法": mzh(m),
                        "校准KL": df.loc[df["method"] == m, "cultural_calibration_kl_mean"].values[0],
                    }
                )
    if not rows:
        print("  SKIPPED")
    ...
    ax.set_xticklabels(piv.index)
    ax.set_ylabel("校准KL")
    ax.set_title("跨4基准校准KL对比")
    ax.legend(fontsize=8)
    ax.grid(axis="y", alpha=0.3)
    save_fig(fig, "cross_suite_calibration")
    save_csv(rdf, "cross_suite_calibration")
...
def fig15_dcas_gains_serp():
    ...
    ax.bar(
        ...
        w,
        label="DCAS-目标校准",
        color=DCAS_CAL_TARGET,
        zorder=3,
    )
    ax.bar(x + w / 2, rdf["BPR-LM"], w, label="BPR-LM", color=BPR_LM, zorder=3)
    ax.set_xticks(x)
    ax.set_xticklabels(rdf["基准"])
    ax.set_ylabel("惊喜度")
    ax.set_title("DCAS-目标校准 vs BPR-LM 惊喜度对比")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
    # Add gain arrows
    for i, (_, row) in enumerate(rdf.iterrows()):
        ax.annotate(
            f"+{row['增益']:.3f}",
            xy=(i, max(row["DCAS-目标校准"], row["BPR-LM"])),
            xytext=(0, 12),
    ...
    save_fig(fig, "dcas_gains_serendipity")
    save_csv(rdf, "dcas_gains_serendipity")
...
def fig16_dcas_gains_minor():
    ...
    for skey, slbl in suites.items():
        ...
        dcas = df.loc[
            df["method"] == "dcas_full_ot_calibrated_target",
            "minority_exposure_at_k_mean",
        ]
        if len(bpr) > 0 and len(dcas) > 0:
            gain = dcas.values[0] - bpr.values[0]
    ...
    ax.set_xticklabels(rdf["基准"])
    ax.set_ylabel("少数曝光率")
    ax.set_title("DCAS-目标校准 vs BPR-LM 少数曝光率对比")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
    for i, (_, row) in enumerate(rdf.iterrows()):
        ax.annotate(
    ...
    save_fig(fig, "dcas_gains_minority")
    save_csv(rdf, "dcas_gains_minority")
...
def fig17_abl_cm_delta():
    ...
    if not comps:
        ...
        return
    ablbl = {
        "full_vs_no_domain": "无领域对抗",
        "full_vs_no_constraints": "无约束",
        "full_vs_no_ot": "无最优传输",
    }
    ...
    for ax, mkey, mlab in zip(axes, mk, ml):
        ...
        for k, v in comps.items():
            ...
            if "delta_mean" in mm:
                names.append(ablbl.get(k, k))
                deltas.append(mm["delta_mean"])
...
def fig18_abl_gm_delta():
    ...
    save_csv(
        pd.DataFrame([{"消融": n, "delta": d} for n, d in zip(names, deltas)]),
        "ablation_gm_delta",
    )
...
def fig19_abl_cm_serp():
    ...
    for k in ["full", "no_domain", "no_constraints", "no_ot"]:
        if k in summ:
            names.append(ABLATION_ZH.get(k, k))
            vals.append(summ[k]["serendipity_mean"])
    fig, ax = plt.subplots(figsize=(8, 4))
    ...
    ax.set_xlabel("惊喜度")
    ax.set_title("CultureMERT 消融: 惊喜度变化")
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.003,
            bar.get_y() + bar.get_height() / 2,
...
def fig21_abl_cm_minor():
    ...
    for k in ["full", "no_domain", "no_constraints", "no_ot"]:
        if k in summ:
            names.append(ABLATION_ZH.get(k, k))
            vals.append(summ[k]["minority_exposure_at_k_mean"])
    fig, ax = plt.subplots(figsize=(8, 4))
    colors = [DCAS_OT, BASELINE_GRAY, BASELINE_LIGHT, POP_ORANGE][: len(names)]
    bars = ax.barh(range(len(names)), vals, color=colors, height=0.6)
    ax.set_yticks(range(len(names)))
    ax.set_yticklabels(names)
    ax.set_xlabel("少数曝光率")
    ax.set_title("CultureMERT 消融: 少数曝光率变化")
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.003,
            bar.get_y() + bar.get_height() / 2,
...
def fig22_abl_gm_minor():
    ...
    ax.set_yticklabels(names)
    ax.set_xlabel("少数曝光率")
    ax.set_title("Gemini 消融: 少数曝光率变化")
    for bar, v in zip(bars, vals):
        ax.text(
            bar.get_width() + 0.003,
            bar.get_y() + bar.get_height() / 2,
...
def fig23_calib_frontier_cm():
    ...
    colors = {
        "dcas_full_ot": DCAS_OT,
        "dcas_ot_cal_p1": DCAS_CAL_TARGET,
        "dcas_ot_cal_p2_target": DCAS_CAL_TARGET,
        "dcas_ot_cal_p3_balanced": DCAS_CAL_MINOR,
        "dcas_ot_cal_p4_minor": DCAS_CAL_MINOR,
        "dcas_ot_cal_p5_ultra_minor": DCAS_CAL_MINOR,
    }
    for _, row in df.iterrows():
        c = colors.get(row["method"], "#888")
        ax.scatter(
            row["minority_exposure_at_k_mean"],
            row["serendipity_mean"],
            s=120,
            c=c,
            zorder=3,
            edgecolors="white",
            lw=1.5,
            label=mzh(row["method"]),
        )
    ax.plot(
        df["minority_exposure_at_k_mean"],
        df["serendipity_mean"],
        color=DCAS_OT,
        lw=2,
        alpha=0.5,
...
def fig24_calib_frontier_ra_cm():
    ...
    save_csv(
        ...
        "calibration_frontier_routeA_cm",
    )
...
def fig25_calib_frontier_ra_gm():
    ...
    save_csv(
        ...
        "calibration_frontier_routeA_gm",
    )
...
def fig26_tradeoff_lines():
    ...
    porder = [
        "dcas_full_ot",
        "dcas_ot_cal_p1",
        "dcas_ot_cal_p2_target",
        "dcas_ot_cal_p3_balanced",
        "dcas_ot_cal_p4_minor",
        "dcas_ot_cal_p5_ultra_minor",
    ]
...
def fig28_baseline_vae():
    ...
    for k in ["vae", "beta_vae", "factorvae", "three_factor_dcas"]:
        if k in vs:
            names.append(METHOD_ZH.get(k, k))
            means.append(vs[k]["serendipity"]["mean"])
    ...
    for ax, mkey, mlab in zip(axes, mk, mkl):
        ...
        for k in ["vae", "beta_vae", "factorvae", "three_factor_dcas"]:
            if k in vs:
                m.append(vs[k][mkey]["mean"])
                s.append(vs[k][mkey]["ci95"])
    ...
    for k in ["vae", "beta_vae", "factorvae", "three_factor_dcas"]:
        if k in vs:
            csv_rows.append(
                {
                    "方法": METHOD_ZH.get(k, k),
                    "惊喜度": vs[k]["serendipity"]["mean"],
                    "校准KL": vs[k]["cultural_calibration_kl"]["mean"],
                    "少数曝光率": vs[k]["minority_exposure_at_k"]["mean"],
                }
            )
...
def fig30_yambda_recall_cov():
    ...
    ax.set_xlabel("覆盖率@10")
    ax.set_ylabel("召回率@10")
    ax.set_title("召回率 vs 覆盖率散点图")
    ax.legend(fontsize=7, loc="best")
    ax.grid(alpha=0.3)
    save_fig(fig, "yambda_recall_vs_coverage")
    save_csv(
        df[["method", "recall_at_10_mean", "coverage_at_10"]],
        "yambda_recall_vs_coverage",
    )
...
def fig31_dataset_culture():
    ...
    save_csv(
        pd.DataFrame({"文化": [cult_zh.get(c, c) for c in cultures], "占比": vals}),
        "dataset_culture_distribution",
    )
...
def fig32_dataset_source():
    ...
    ax.set_xticklabels(list(sources.keys()), fontsize=9)
    ax.set_ylabel("数据集数量")
    ax.set_title("V4 数据集来源分布")
    ...
    save_csv(
        pd.DataFrame({"来源": list(sources.keys()), "数量": list(sources.values())}),
