...
COLORS = {
    'dcas_ot': '#4ecca3', 'dcas_cal': '#2ba582', 'dcas_min': '#1a8a6e',
    'bpr_lm': '#0f3460', 'bpr_light': '#3a7ca5',
    'baseline': '#888888', 'baseline_light': '#aaaaaa',
    'pop': '#d65a31', 'cosine': '#e8a87c', 'knn': '#e8c07c',
    'lightfm': '#c38d9e',
}
METHOD_NAMES = {
    'popularity': 'Popularity', 'cosine': 'Cosine', 'knn': 'KNN',
    'lightfm_like': 'LightFM', 'bpr_mf': 'BPR-MF',
    'bpr_two_stage_hybrid': 'BPR-2Stage', 'bpr_listwise_hybrid': 'BPR-Listwise',
    'bpr_lambdamart_hybrid': 'BPR-LM', 'dcas_full_ot': 'DCAS-OT',
    'dcas_full_ot_calibrated_target': 'DCAS-Cal',
    'dcas_full_ot_calibrated_minor': 'DCAS-Min', 'dcas_log_ot': 'DCAS-Log-OT',
}
SUITE_LABELS = {
...
CAL_NAMES = {
    ...
    'dcas_ot_cal_p1': 'P1 (Serendipity)', 'dcas_ot_cal_p2_target': 'P2 (Balanced)',
...
def method_color(m):
    if m in ('dcas_full_ot',): return COLORS['dcas_ot']
    if m in ('dcas_full_ot_calibrated_target',): return COLORS['dcas_cal']
    if m in ('dcas_full_ot_calibrated_minor',): return COLORS['dcas_min']
    if m in ('dcas_log_ot',): return COLORS['dcas_min']
    if m == 'bpr_lambdamart_hybrid': return COLORS['bpr_lm']
    if m.startswith('bpr'): return COLORS['bpr_light']
    if m == 'popularity': return COLORS['pop']
    if m == 'cosine': return COLORS['cosine']
    if m == 'knn': return COLORS['knn']
    if m == 'lightfm_like': return COLORS['lightfm']
    return COLORS['baseline']

def get_methods_data(data, metrics=None):
    if metrics is None:
        metrics = ['serendipity_mean', 'cultural_calibration_kl_mean', 'minority_exposure_at_k_mean']
...
def fig_cross_suite(benchmarks, metric, png_name, title):
    suites = ['v4_main_culturemert_stage3_lambdamart', 'v4_main_gemini_stage3_lambdamart',
              'v4_routeA_small_culturemert_stage3_lambdamart', 'v4_routeA_small_gemini_stage3_lambdamart']
    common_methods = ['popularity', 'bpr_mf', 'bpr_two_stage_hybrid', 'bpr_listwise_hybrid',
                       'bpr_lambdamart_hybrid', 'dcas_full_ot', 'dcas_full_ot_calibrated_target',
    ...
    if not data_matrix:
        print(f"  SKIPPED: {png_name}")
        return
    df_cross = pd.DataFrame(data_matrix, columns=[METHOD_NAMES.get(m, m) for m in common_methods], index=suite_labels)
...
def fig_dcas_gains(benchmarks, metric, png_name, title):
    ...
    bars = ax.barh(range(len(gains)), gains, color=colors, edgecolor='white')
...
def fig_ablation_delta(abl_data, png_name, backbone_name):
    ...
    for ax, met in zip(axes, metrics_abl):
        ...
        ax.barh(y_pos, deltas, xerr=[cis_low, cis_high], color=colors, edgecolor='white', capsize=3)
        ax.set_yticks(y_pos)
        ax.set_yticklabels(labels)
    ...
    for key, (label, comp_data) in variants.items():
        ...
        for met in metrics_abl:
            met_data = comp_data.get('metrics', {}).get(met, {})
            rows.append({'variant': label, 'metric': met, 'delta': met_data.get('delta_mean', 0),
                         'ci_low': met_data.get('delta_ci95_low', 0), 'ci_high': met_data.get('delta_ci95_high', 0)})
...
def fig_ablation_values(abl_data, png_name, backbone_name, metric_key, title_suffix):
    ...
    for key in ['full', 'no_domain', 'no_constraints', 'no_ot']:
        lbl = {'full': 'Full (Ours)', 'no_domain': 'No Domain', 'no_constraints': 'No Constraints', 'no_ot': 'No OT'}.get(key, key)
        val = eval_sum.get(key, {}).get(metric_key, np.nan)
        if not np.isnan(val):
            labels.append(lbl)
            values.append(val)
    colors = [COLORS['dcas_ot'], COLORS['baseline'], COLORS['baseline_light'], COLORS['pop']]
...
def fig_calibration_frontier(data, png_name, title):
    methods = data.get('methods', {})
    cal_methods = ['dcas_full_ot', 'dcas_full_ot_calibrated_target', 'dcas_full_ot_calibrated_minor',
                   'bpr_lambdamart_hybrid', 'popularity']
    points = []
    ...
    for _, p in df_p.iterrows():
        c = {'DCAS-OT': COLORS['dcas_ot'], 'DCAS-Cal': COLORS['dcas_cal'], 'DCAS-Min': COLORS['dcas_min'],
             'BPR-LM': COLORS['bpr_lm'], 'Popularity': COLORS['pop']}.get(p['method'], '#888888')
        ax.scatter(p['serendipity'], p['minority'], s=150, c=c, edgecolors='black', linewidth=0.5, zorder=5)
        ax.annotate(p['method'], (p['serendipity'], p['minority']), textcoords="offset points",
...
def fig_calibration_tradeoff(data, png_name, title):
    ...
    metrics_t = ['serendipity_mean', 'cultural_calibration_kl_mean', 'minority_exposure_at_k_mean']
...
def fig_radar(data, png_name, title):
    ...
    radar_methods = ['popularity', 'bpr_lambdamart_hybrid', 'dcas_full_ot', 'dcas_full_ot_calibrated_target']
    metrics_radar = ['serendipity_mean', 'cultural_calibration_kl_mean', 'minority_exposure_at_k_mean']
    radar_colors = [COLORS['pop'], COLORS['bpr_lm'], COLORS['dcas_ot'], COLORS['dcas_cal']]
...
def fig_vae_comparison(baseline_data, png_name):
    ...
    for ax, met in zip(axes, metrics_vae):
        ...
        if not means:
            continue
        ax.barh(range(len(means)), means, xerr=ci95s, color=colors_used, edgecolor='white', capsize=4)
...
def fig_yambda_4panel(data, png_name):
    if not data:
        ...
        return
    methods = data.get('methods', {})
    metrics_y = ['recall_at_10_mean', 'ndcg_at_10_mean', 'mrr_at_10_mean', 'coverage_at_10']
...
def fig_yambda_recall_vs_coverage(data, png_name):
    ...
    for _, p in df_yp.iterrows():
        c = {'Popularity': COLORS['pop'], 'Cosine': COLORS['cosine'], 'KNN': COLORS['knn'],
             'BPR-MF': COLORS['bpr_light'], 'BPR-LM': COLORS['bpr_lm'],
             'DCAS-OT': COLORS['dcas_ot'], 'DCAS-Cal': COLORS['dcas_cal']}.get(p['method'], '#888888')
        ax.scatter(p['coverage'], p['recall'], s=150, c=c, edgecolors='black', linewidth=0.5, zorder=5)
        ax.annotate(p['method'], (p['coverage'], p['recall']), textcoords="offset points", xytext=(8, 5), fontsize=8)
...
def fig_culture_distribution(png_name):
    ...
    for bar, cnt in zip(bars, counts):
        ax.text(bar.get_x() + bar.get_width() / 2, cnt + 50, str(cnt), ha='center', fontsize=9)
    fig.tight_layout()
...
def fig_pal_trajectory(pal_data, png_name):
    ...
    fig, axes = plt.subplots(1, 3, figsize=(14, 4))
    metrics_pal = [(serendipity_vals, 'Serendipity'), (kl_vals, 'Calibration KL'), (minority_vals, 'Minority@k')]
    for ax, (vals, lbl) in zip(axes, metrics_pal):
        ...
        ax.plot(xlabs, yvals, marker='o', linewidth=2, markersize=8, color=COLORS['dcas_cal'])
        ax.set_ylabel(lbl)
        ax.set_xlabel('PAL Round')
    fig.suptitle('PAL Training Trajectory', fontsize=13, fontweight='bold')
    fig.tight_layout()
    df_pal = pd.DataFrame({'round': labels_data, 'serendipity': serendipity_vals,
                           'calibration_kl': kl_vals, 'minority_exposure': minority_vals})
...
def main():
    ...
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_3panel_results(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                          'v4_cm_main_results_3panel.png', 'V4 CultureMERT')

    print("[2/33] V4 Gemini 3-panel results...")
    if 'v4_main_gemini_stage3_lambdamart' in benchmarks:
        fig_3panel_results(benchmarks['v4_main_gemini_stage3_lambdamart'],
                          'v4_gm_main_results_3panel.png', 'V4 Gemini')

    # Figures 3-4: routeA
    print("[3/33] RouteA CultureMERT results...")
    if 'v4_routeA_small_culturemert_stage3_lambdamart' in benchmarks:
        fig_routeA_results(benchmarks['v4_routeA_small_culturemert_stage3_lambdamart'],
                          'routeA_cm_main_results.png', 'RouteA CultureMERT')

    print("[4/33] RouteA Gemini results...")
    if 'v4_routeA_small_gemini_stage3_lambdamart' in benchmarks:
        fig_routeA_results(benchmarks['v4_routeA_small_gemini_stage3_lambdamart'],
                          'routeA_gm_main_results.png', 'RouteA Gemini')

    # Figure 5: public CN
    print("[5/33] Public RouteA CN results...")
    if 'public_routeA_phase2_cn_lambdamart' in benchmarks:
        fig_public_cn_results(benchmarks['public_routeA_phase2_cn_lambdamart'],
                             'public_cn_results.png')

    # Figures 6-7: Serendipity all methods
    print("[6/33] All-methods serendipity (CM)...")
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_all_methods_single(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                               'serendipity_all_methods_cm.png',
                               'serendipity_mean', 'V4 CultureMERT - Serendipity by Method')

    print("[7/33] All-methods serendipity (GM)...")
    if 'v4_main_gemini_stage3_lambdamart' in benchmarks:
        fig_all_methods_single(benchmarks['v4_main_gemini_stage3_lambdamart'],
                               'serendipity_all_methods_gm.png',
                               'serendipity_mean', 'V4 Gemini - Serendipity by Method')
    ...
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_all_methods_single(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                               ...
                               'minority_exposure_at_k_mean', 'V4 CultureMERT - Minority Exposure')

    print("[9/33] All-methods minority (GM)...")
    if 'v4_main_gemini_stage3_lambdamart' in benchmarks:
        fig_all_methods_single(benchmarks['v4_main_gemini_stage3_lambdamart'],
                               'minority_all_methods_gm.png',
                               'minority_exposure_at_k_mean', 'V4 Gemini - Minority Exposure')
    ...
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_all_methods_single(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                               ...
                               'cultural_calibration_kl_mean', 'V4 CultureMERT - Calibration KL')

    print("[11/33] All-methods calibration (GM)...")
    if 'v4_main_gemini_stage3_lambdamart' in benchmarks:
        fig_all_methods_single(benchmarks['v4_main_gemini_stage3_lambdamart'],
                               'calibration_all_methods_gm.png',
                               'cultural_calibration_kl_mean', 'V4 Gemini - Calibration KL')

    # Figures 12-14: Cross-suite
    print("[12/33] Cross-suite serendipity...")
    fig_cross_suite(benchmarks, 'serendipity_mean', 'cross_suite_serendipity.png',
                    'Cross-Benchmark Serendipity')

    print("[13/33] Cross-suite minority...")
    fig_cross_suite(benchmarks, 'minority_exposure_at_k_mean', 'cross_suite_minority.png',
                    'Cross-Benchmark Minority Exposure')

    print("[14/33] Cross-suite calibration...")
    fig_cross_suite(benchmarks, 'cultural_calibration_kl_mean', 'cross_suite_calibration.png',
                    'Cross-Benchmark Calibration KL')

    # Figures 15-16: DCAS gains
    print("[15/33] DCAS serendipity gains...")
    fig_dcas_gains(benchmarks, 'serendipity_mean', 'dcas_gains_serendipity.png',
                   'DCAS-Cal vs BPR-LM: Serendipity Gains')

    print("[16/33] DCAS minority gains...")
    fig_dcas_gains(benchmarks, 'minority_exposure_at_k_mean', 'dcas_gains_minority.png',
    ...
    fig_ablation_values(ablation_cm, 'ablation_cm_serendipity.png', 'CultureMERT',
                        'serendipity_mean', 'Serendipity')

    print("[20/33] Gemini ablation serendipity...")
    fig_ablation_values(ablation_gm, 'ablation_gm_serendipity.png', 'Gemini',
                        'serendipity_mean', 'Serendipity')

    # Figures 21-22: Ablation minority
    print("[21/33] CultureMERT ablation minority...")
    fig_ablation_values(ablation_cm, 'ablation_cm_minority.png', 'CultureMERT',
                        'minority_exposure_at_k_mean', 'Minority@k')

    print("[22/33] Gemini ablation minority...")
    fig_ablation_values(ablation_gm, 'ablation_gm_minority.png', 'Gemini',
                        'minority_exposure_at_k_mean', 'Minority@k')

    # Figure 23: Calibration frontier CM
    print("[23/33] CultureMERT calibration frontier...")
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_calibration_frontier(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                                 'calibration_frontier_cm.png', 'V4 CultureMERT')

    # Figures 24-25: Calibration frontier routeA
    print("[24/33] RouteA CM calibration frontier...")
    if 'v4_routeA_small_culturemert_stage3_lambdamart' in benchmarks:
        fig_calibration_frontier(benchmarks['v4_routeA_small_culturemert_stage3_lambdamart'],
                                 'calibration_frontier_routeA_cm.png', 'RouteA CultureMERT')

    print("[25/33] RouteA GM calibration frontier...")
    if 'v4_routeA_small_gemini_stage3_lambdamart' in benchmarks:
        fig_calibration_frontier(benchmarks['v4_routeA_small_gemini_stage3_lambdamart'],
                                 'calibration_frontier_routeA_gm.png', 'RouteA Gemini')

    # Figure 26: Calibration tradeoff
    print("[26/33] Calibration tradeoff lines...")
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_calibration_tradeoff(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                                 'calibration_tradeoff_lines.png', 'V4 CultureMERT')

    # Figure 27: Radar
    print("[27/33] 4-method radar chart...")
    if 'v4_main_culturemert_stage3_lambdamart' in benchmarks:
        fig_radar(benchmarks['v4_main_culturemert_stage3_lambdamart'],
                  'radar_4method.png', 'V4 CultureMERT - Method Radar')
    ...
    if 'yambda_5b_subset_global_log_benchmark' in benchmarks:
        fig_yambda_4panel(benchmarks['yambda_5b_subset_global_log_benchmark'],
                          'yambda_log_benchmark_4panel.png')

    print("[30/33] Yambda recall vs coverage...")
    if 'yambda_5b_subset_global_log_benchmark' in benchmarks:
        fig_yambda_recall_vs_coverage(benchmarks['yambda_5b_subset_global_log_benchmark'],
