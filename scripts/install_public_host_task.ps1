param(
    [string]$TaskName = "Echo Public Host",
    [int]$Port = 8000,
...
$scriptPath = Join-Path $repoRoot "scripts\run_public_host.ps1"
...
try {
    Register-ScheduledTask `
        ...
        -Trigger $trigger `
        -Settings $settings `
        -Description "Run Echo FastAPI and static frontend on this Windows host." `
        -Force `
...
$shortcut.Description = "Run Echo FastAPI and static frontend on this Windows host."
$shortcut.Save()

Write-Host "Installed Startup shortcut: $shortcutPath"
Write-Host "Start Echo now with:"
